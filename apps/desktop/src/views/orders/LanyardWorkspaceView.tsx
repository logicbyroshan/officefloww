...
function generateId(): string {
  return `lanyard-${_idCounter++}`;
}

const STORAGE_KEY = "officefloww_lanyard_orders_v1";

// ─── Seed data ────────────────────────────────────────────────────────────────

const SEED: LanyardEntry[] = [
  {
    id: "lanyard-seed-1",
    date: "2026-08-28",
    details: "Tata Motors — 16mm Satin Lanyard, Royal Blue with White Print, Premium Dog Hook",
    qty: 500,
    goneForPrint: true,
    isPrinted: true,
  ...
  },
  {
    id: "lanyard-seed-2",
    date: "2026-09-01",
    details: "Infosys Ltd — 20mm Multicolor Sublimation Lanyard, Fish Hook + Safety Breakaway",
    qty: 250,
    goneForPrint: true,
    isPrinted: true,
    goneForFitting: true,
  },
  {
    id: "lanyard-seed-3",
    date: "2026-09-02",
    details: "HDFC Bank — 12mm Flat Polyester Lanyard, Navy Blue Screen Print, Alligator Clip",
    qty: 1000,
    goneForPrint: true,
    isPrinted: false,
    goneForFitting: false,
  },
  {
    id: "lanyard-seed-4",
    date: "2026-09-04",
    details: "DPS School — 16mm Single Color Screen Print, Oval Hook + Release Buckle",
    qty: 350,
    goneForPrint: false,
    isPrinted: false,
    goneForFitting: false,
...
export const LanyardWorkspaceView: React.FC = () => {
  ...
  const [entries, setEntries] = useState<LanyardEntry[]>(() => {
    try {
  ...
  const handleAdd = () => {
    ...
    setNewDetails("");
    setNewQty("");
    toastSuccess("Lanyard order added.");
  };

  const handleCheckChange = (id: string, field: keyof LanyardEntry, value: boolean) => {
    setEntries((prev) =>
  ...
  const handleQtyChange = (id: string, val: number) => {
    setEntries((prev) =>
      prev.map((e) => (e.id === id ? { ...e, qty: Math.max(1, val) } : e))
    );
  };
    ...
    >
      ...
      >
        <div>
          ...
          >
            {filtered.length} entries · {totalQty.toLocaleString()} total lanyards
          </p>
        </div>
      ...
      >
        <input
          type="text"
          placeholder="Order details — e.g. Tata Motors, 16mm Satin, Royal Blue, Dog Hook…"
          value={newDetails}
        ...
        <input
          ...
          value={newQty}
          onChange={(e) =>
            setNewQty(e.target.value === "" ? "" : parseInt(e.target.value, 10))
          }
          onKeyDown={(e) => e.key === "Enter" && handleAdd()}
          style={{
            width: "90px",
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                ...
                >
                  No Lanyard entries yet. Add one above.
                </td>
              </tr>
            ) : (
              filtered.map((entry, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Checkboxes */}
                    <CheckCell
                      checked={entry.goneForPrint}
                      onChange={(v) =>
                        handleCheckChange(entry.id, "goneForPrint", v)
                      }
                      label="Gone for Print"
                      colorOn="#f59e0b"
                    />
                    <CheckCell
                      checked={entry.isPrinted}
                      onChange={(v) =>
                        handleCheckChange(entry.id, "isPrinted", v)
                      }
                      label="Is Printed"
                      colorOn="#60a5fa"
                    />
                    <CheckCell
                      checked={entry.goneForFitting}
                      onChange={(v) =>
                        handleCheckChange(entry.id, "goneForFitting", v)
                      }
                      label="Gone for Fitting"
                      colorOn="#34d399"
                    />
